...
public class SoulCage extends Block implements ITileEntityProvider
{
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int direction, float par7, float par8, float par9)
    {
        ...
        {
        	if (tile.inventory == null) return false;
            world.spawnEntityInWorld(new EntityItem(world, GetOffset(direction, x, 'x'), GetOffset(direction, y, 'y'), GetOffset(direction, z, 'z'), tile.decrStackSize(0, 1)));
        }
		return false;
    }
