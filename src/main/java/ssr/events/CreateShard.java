...
public class CreateShard {
	...
	public void createShard(PlayerInteractEvent event) {
		...
		if (player.getHeldItem().getItem() == Items.diamond) {
			...
			if (block == Blocks.glowstone
					&& (isFormed(world, event.x, event.y, event.z) || isFormedVertically(
						...
						if (isNorthSouthOrientation) {
							for (int i = -1; i < 2; i++) {
								for (int ii = -1; ii < 2; ii++) {
									world.setBlockToAir(event.x, event.y + ii,
											event.z + i);
									if (ii == 0 && i == -1)
										world.setBlockToAir(event.x, event.y,
												event.z - 2);
									if (ii == 0 && i == 1)
										world.setBlockToAir(event.x, event.y,
												event.z + 2);
							...
							for (int i = -1; i < 2; i++) {
								for (int ii = -1; ii < 2; ii++) {
									world.setBlockToAir(event.x + i, event.y
											+ ii, event.z);
									if (ii == 0 && i == -1)
										world.setBlockToAir(event.x - 2,
												event.y, event.z);
									if (ii == 0 && i == 1)
										world.setBlockToAir(event.x + 2,
												event.y, event.z);
						...
						for (int i = -1; i < 2; i++) {
							for (int ii = -1; ii < 2; ii++) {
								world.setBlockToAir(event.x + i, event.y,
										event.z + ii);
								if (ii == 0 && i == -1) {
									world.setBlockToAir(event.x - 2, event.y,
											event.z);
									world.setBlockToAir(event.x, event.y,
											event.z - 2);
								}
	...
	private boolean isFormed(World world, int x, int y, int z) {
		Block blocks1[] = { world.getBlock(x + 1, y, z),
				world.getBlock(x - 1, y, z), world.getBlock(x, y, z + 1),
				world.getBlock(x, y, z - 1) };
		Block blocks2[] = { world.getBlock(x + 2, y, z),
				world.getBlock(x - 2, y, z), world.getBlock(x, y, z + 2),
				world.getBlock(x, y, z - 2), world.getBlock(x + 1, y, z + 1),
	...
	private boolean isFormedVertically(World world, int x, int y, int z) {
		boolean result;
		Block blocks1[] = { world.getBlock(x, y + 1, z),
				world.getBlock(x, y - 1, z), world.getBlock(x, y, z + 1),
				world.getBlock(x, y, z - 1) };
		Block blocks2[] = { world.getBlock(x, y + 1, z - 1),
				world.getBlock(x, y - 1, z - 1),
				world.getBlock(x, y + 1, z + 1),
				world.getBlock(x, y - 1, z + 1), world.getBlock(x, y + 2, z),
				world.getBlock(x, y - 2, z), world.getBlock(x, y, z + 2),
		...
		Block blocks3[] = { world.getBlock(x, y + 1, z),
				world.getBlock(x, y - 1, z), world.getBlock(x + 1, y, z),
				world.getBlock(x + 1, y, z) };
		Block blocks4[] = { world.getBlock(x - 1, y + 1, z),
				world.getBlock(x - 1, y - 1, z),
				world.getBlock(x + 1, y + 1, z),
				world.getBlock(x + 1, y - 1, z), world.getBlock(x, y + 2, z),
				world.getBlock(x, y - 2, z), world.getBlock(x + 2, y, z),
